...
3. Lines 46 + 161-163: the module-level default `REG_NORM = None` (line 46) is passed into `Glove(...)` at line 276, which then routes the regularization through the `if self.reg_norm is None:` branch at lines 161-163, computing `1 - torch.nn.functional.cosine_similarity(...)`. The paper specifies **squared L2 norm**. Equation 4 (p. 5) and Algorithm 1 (p. 14) both define the regularization term as `λ Σᵢ ‖wᵢ − wⁿ²ᵛᵢ‖²`. Pass `reg_norm=2` when constructing the `Glove` model so the regularization term takes the L2 branch at lines 165-166 (`torch.norm(..., p=self.reg_norm, dim=1)`) instead of `1 - cosine_similarity`.
